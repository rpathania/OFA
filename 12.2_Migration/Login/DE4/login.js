...
var isLocked=false;
		...
		str4 = null;
      str4 = SessionMgr.createAppsSession(userPwd, paramHttpServletRequest, paramHttpServletResponse);
...
function submitCredentials() {
    ...
    spinner(true);  
/*Added by Rishi PATHANIA*/
	console.log("Calling the authenticate servlet");
...
function createLanguageDropBox(id ) {
    ...
    if (box.length==0) {
        for(l in appsMessage) {
            ...
            {
		if ( appsMessage[l]['LANG/NAME']=='English') {
                var opt ;
                if (browser.isIE) {
